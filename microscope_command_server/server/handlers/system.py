...
def handle_siftal(conn, client, hardware, settings, **kwargs):
    ...
    while i < len(parts):
        if parts[i] == "--wsi-region" and i + 1 < len(parts):
            ...
            i += 2
        elif parts[i] == "--rgb-conv" and i + 1 < len(parts):
            params["rgb_conversion"] = parts[i + 1]
            i += 2
        elif parts[i] == "--flip-x":
            params["flip_x"] = True
            i += 1
