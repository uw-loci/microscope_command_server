...
def handle_sensitivity_test(
    ...
    log.info("Starting PPM sensitivity test (type=%s, base_angle=%s, repeats=%s)",
             params["test_type"], params["base_angle"], params["n_repeats"])
...
def handle_birefringence_test(
    ...
    log.info(
        ...
        params["angle_step"], params["exposure_mode"],
    )
    ...
    result_dir = run_birefringence_maximization_test(
        ...
        stage_move_callback=(
            stage_move_callback
            if params["exposure_mode"] == "calibrate"
            else None
        ),
    )
    return result_dir
